...
This module intentionally treats a counter as a *repeatable winning plan*, not
as a Pokémon that merely shares a favourable type-chart interaction.

The model has several independent evidence layers:

* legal Champions learnset pressure
* move category/power/priority information from PokéAPI
* offensive and defensive type interactions
* target STAB pressure and candidate survivability
* speed / priority windows
* ability-based matchup modifiers
* tournament usage, win-rate and top-cut evidence
* tournament co-occurrence as weak team-context evidence only
* optional tournament move-frequency evidence for future history data

The tournament-move hooks are deliberately schema-tolerant so the history
pipeline can add move usage without requiring another rewrite of this engine.
"""
...
def _fetch_move_metadata(move_name: str) -> Dict[str, Any]:
    """Return compact move metadata used by the counter model."""
    slug = str(move_name or "").strip().lower().replace(" ", "-").replace("'", "").replace(".", "")
    if not slug:
        return {}
    try:
...
def _normalise_usage(value: Any) -> float:
    try:
        ...
        return 0.0
    # History percentages occasionally arrive as 0-100 while tournament_score
    # and rates arrive as 0-1.
    if number > 1.0:
        number /= 100.0
...
def _observed_moves(candidate_data: Mapping[str, Any]) -> Dict[str, float]:
    """Read optional tournament move-frequency evidence.

    Supported shapes include::

        {"tournament_moves": {"Close Combat": 0.81}}
        {"move_usage": {"moves": {"Close Combat": 81}}}

    The engine never requires this data; when absent it falls back to the
    verified Champions learnset supplied by ``fetch_pokemon_details``.
    """
...
def _learnset_moves(data: Mapping[str, Any]) -> List[str]:
    """Only use moves actually supplied by the Champions data layer."""
...
def _pressure_moves(
    ...
    """Score actual learnset moves, prioritising real damaging routes."""
    moves = _learnset_moves(candidate)
    observed = _observed_moves(candidate)
    if observed:
        # Observed moves are allowed to override the raw learnset only when the
        # move is also present in the legal Champions learnset. This prevents a
        # future stats feed from accidentally introducing illegal moves.
    ...
    for move in moves:
        ...
        # A lightweight proxy for expected pressure. It deliberately avoids
        # pretending to be exact battle damage because EVs, items, natures and
        # spread-specific mechanics are not yet in this layer.
        ...
        rows.append({
            ...
            "frequency": frequency,
            "pressure": pressure,
            "stab": _is_stab(move_type, candidate_types),
        })
    ...
    if rows:
        ...
        if observed and best["frequency"] > 0:
            reasons.append(
                f"Tournament-used pressure move: {best['move']} ({best['frequency'] * 100:.0f}% observed)"
            )
        else:
...
def _ability_defensive_bonus(candidate: Mapping[str, Any], target_types: Sequence[str]) -> Tuple[float, List[str]]:
    ...
    if "intimidate" in abilities and "attack" in " ".join(str(x).lower() for x in target_types):
        bonus += 0.0
...
def _target_damaging_stabs(target: Mapping[str, Any], candidate_types: Sequence[str]) -> List[Dict[str, Any]]:
    """Estimate the target's strongest STAB attacks from its legal learnset."""
    rows: List[Dict[str, Any]] = []
...
def _tournament_score(name: str) -> Tuple[float, float, Dict[str, Any]]:
    ...
    # 50% win rate is neutral. We reward sustained evidence, not raw volume.
...
def _team_context_score(
    ...
    target_partner_counts: Mapping[str, int],
) -> Tuple[float, List[str]]:
    """Use co-occurrence as context, never as direct counter proof."""
    ...
    reasons: List[str] = []
    if pair_count:
        reasons.append(f"Recorded tournament co-occurrence with the target: {pair_count} teams")
...
def assess_counter(
    ...
    *,
    ...
    """Build a practical, evidence-weighted matchup assessment."""
    ...
    # Move evidence is the most important correction versus the previous model:
    # typing alone cannot create a Hard Counter.
    if move_rows:
    ...
    # Offensive score requires both type pressure and a real move route.
    if best_effectiveness >= 4.0:
        type_offense = 100.0
    elif best_effectiveness >= 2.0:
        type_offense = 78.0
    ...
    # Archetype compatibility is useful, but intentionally capped so it cannot
    # manufacture a counter from a good team role.
    ...
    # A candidate is a practical counter only if it has a credible offensive
    # route AND is not overwhelmingly vulnerable to the target's best STAB.
    ...
    if best_attack >= 4.0 and best_effectiveness < 2.0:
        warnings.append("Typing looks excellent, but no verified damaging move currently proves the matchup")
    if target_attack_multiplier >= 2.0 and speed < 70.0 and defensive < 55.0:
        warnings.append("Target can likely win the return interaction before this candidate establishes pressure")
    if candidate_speed >= target_speed:
        reasons.append("Base Speed gives this candidate a realistic first-action window")
    if priority_score:
        reasons.append("Priority can preserve the pressure window against faster targets")
    if tournament >= 60:
        reasons.append("Strong current tournament evidence supports practical use")
    elif tournament < 18:
        warnings.append("Limited tournament evidence")

    # Strong pairing evidence is deliberately a small contextual factor.
    # It can never rescue a bad direct matchup.
    evidence = (
        matchup * 0.40
        + survival * 0.22
        + tournament * 0.22
    ...
    # Evidence confidence is separate from matchup score. A theoretical monster
    # with no tournament support remains usable, but should not look certain.
    confidence = max(
        0.08,
        min(
            1.0,
            0.30
            + tournament_confidence * 0.35
            + (0.20 if move_rows else 0.0)
            + (0.10 if best_effectiveness >= 2.0 else 0.0)
            + (0.05 if target_attack_multiplier <= 1.0 else 0.0),
        ),
    )

    score = evidence * (0.70 + 0.30 * confidence)

    # Hard gates. These are what stop Gallade/Medicham-style type-chart
    # accidents from becoming top recommendations.
    hard_route = best_effectiveness >= 2.0 and move_quality >= 48.0
    safe_route = defensive >= 58.0 or speed >= 88.0
    if hard_route and safe_route:
        category = "Hard Counter" if defensive >= 65.0 or speed >= 88.0 else "Soft Counter"
    elif hard_route or (speed >= 88.0 and move_quality >= 55.0 and defensive >= 42.0):
        category = "Soft Counter"
    elif matchup >= 62.0 and team_context >= 18.0:
    ...
    if best_effectiveness < 2.0 and move_quality < 55.0:
        category = "Matchup Check"
        score *= 0.72
        warnings.append("No strong verified super-effective damage route")

    if target_attack_multiplier >= 2.0 and defensive < 45.0 and speed < 88.0:
        category = "Matchup Check"
        score *= 0.70
        warnings.append("Fails the survivability / tempo gate against the target")

    if best_effectiveness >= 2.0:
        reasons.append(f"Verified {best_effectiveness:g}× offensive type pressure into the target")
    if target_attack_multiplier <= 0.5:
        reasons.append(f"Target's strongest STAB typing is only {target_attack_multiplier:g}× into this candidate")

    best_moves = [row["move"] for row in move_rows[:3]]
    if not best_moves:
        best_moves = []
...
def rank_counters(
    ...
    *,
    ...
    assessments.sort(
        key=lambda item: (item.score, item.confidence, item.matchup, item.tournament),
        reverse=True,
    )

    # Select the strongest answers first. Diversity is only a tie-breaker: it
    # never promotes a materially weaker Pokémon just to make the list pretty.
    ...
    # If strict diversity left the result short, fill from the remaining best
    # answers. Quality always beats artificial variety.
