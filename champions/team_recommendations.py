...
from champions.canonical import canonical_species_key
...
def _candidate_shortlist(active_names: Sequence[str], limit: int = 36) -> List[str]:
    ...
    for species_key, raw_record in records.items():
        ...
        relevance = (
            min(36.0, appearances / 18.0)
            + min(24.0, usage * 80.0)
            + min(20.0, top_cut * 40.0)
            + min(20.0, recent * 20.0)
        )
    ...
    # Preserve strong observed partnerships even when a partner's raw usage is modest.
    ...
    for active_name in active_names:
        try:
            for species_key, _count in get_tournament_partners(active_name, top_n=8) or []:
                ...
                if (
                    partner
                    and not partner.lower().startswith("mega ")
                    and canonical_species_key(partner) not in active_keys
                ):
...
def _team_types(team: Sequence[Mapping[str, Any]]) -> Counter[str]:
    return Counter(
        str(team_member.get("types")[i]).title()
        for team_member in team
        for i in range(len(team_member.get("types") or []))
    )
...
def _reason_summary(
    ...
    before_def = set(base["defensive"]["uncovered_types"])
    after_def = set(improved["defensive"]["uncovered_types"])
    solved = sorted(before_def - after_def)
    if solved:
        reasons.append("Adds answers to " + ", ".join(solved[:3]))

    before_off = set(base["offensive"]["uncovered_types"])
    after_off = set(improved["offensive"]["uncovered_types"])
    opened = sorted(before_off - after_off)
    if opened:
        reasons.append("Opens super-effective pressure into " + ", ".join(opened[:3]))

    base_functions = base["functions"]
    new_functions = improved["functions"]
    ...
    for key, label in (
        ...
        before = set(base_functions.get(key) or [])
        after = set(new_functions.get(key) or [])
        if after - before:
            newly_added.append(label)
    if newly_added:
        reasons.append("Adds " + ", ".join(newly_added[:3]))

    base_types = _team_types(base["team"] if "team" in base else [])
...
def _meta_relevance(name: str) -> float:
    ...
    return max(
        ...
        min(
            100.0,
            float(current.get("usage", 0) or record.get("usage", 0) or 0) * 100.0 * 0.55
            + float(current.get("top_cut_rate", 0) or record.get("top_cut_rate", 0) or 0) * 100.0 * 0.25
...
def _rank_candidate(
    ...
    def_delta = len(base["defensive"]["uncovered_types"]) - len(improved["defensive"]["uncovered_types"])
    off_delta = len(base["offensive"]["uncovered_types"]) - len(improved["offensive"]["uncovered_types"])
    function_delta = improved["functions"]["score"] - base["functions"]["score"]
    archetype_delta = improved["archetypes"]["score"] - base["archetypes"]["score"]
    meta = _meta_relevance(str(candidate.get("name")))

    improvement = (
        delta * 0.52
        + max(0.0, def_delta) * 4.0
        + max(0.0, off_delta) * 3.0
        + max(0.0, function_delta) * 0.14
        + max(0.0, archetype_delta) * 0.08
        + meta * 0.10
    )

    details = {
        "score": round(max(0.0, min(100.0, improvement)), 1),
        ...
        "reasons": _reason_summary(base, improved, candidate),
    }
...
def recommend_team_additions(
    ...
    *,
    ...
    names = [str(member["name"]) for member in active_team]
    candidate_names = _candidate_shortlist(names, limit=candidate_limit)
    ...
    for name in candidate_names:
        ...
        candidate_team = active_team + [candidate]
        improved = TeamAnalyzer(candidate_team).analyze()
        improved["team"] = candidate_team
        score, detail = _rank_candidate(
            float(base_result["overall_score"]),
            float(improved["overall_score"]),
            base_result,
            improved,
            candidate,
        )
