...
def _bar_colour(value: int) -> str:
    """Return a continuous red -> orange -> green colour for a final stat."""
    low, high = 50.0, 150.0
    t = max(0.0, min(1.0, (float(value) - low) / (high - low)))
    stops = ((0.0, (239, 68, 68)), (0.5, (249, 145, 0)), (1.0, (122, 199, 76)))
    if t <= 0.5:
        a_pos, a = stops[0]; b_pos, b = stops[1]
    else:
        a_pos, a = stops[1]; b_pos, b = stops[2]
    p = (t - a_pos) / (b_pos - a_pos)
    rgb = tuple(round(a[i] + (b[i] - a[i]) * p) for i in range(3))
...
def _stat_bar_html(value: int) -> str:
    ...
    return f'''<div style="width:100%;height:22px;border-radius:999px;background:#263241;border:1px solid #526071;overflow:hidden;box-sizing:border-box;">
<div style="width:{width}%;height:100%;background:{colour};border-radius:999px;box-shadow:0 0 7px {colour}88;"></div></div>'''
...
def render_dynamic_stat_graph(slot_index: int, pokemon_name: str, base_stats: Optional[Mapping[str, Any]], nature: str) -> Dict[str, int]:
    ...
    for label, key in _STAT_KEYS:
        ...
        cols[1].markdown(f"<span style='color:#f1f5f9'>{base}</span>", unsafe_allow_html=True)
        cols[2].markdown(f"<span style='color:#f1f5f9'>+{ev}</span>", unsafe_allow_html=True)
        cols[3].markdown(f"<span style='color:#f1f5f9'>{nature_text}</span>", unsafe_allow_html=True)
        cols[4].html(_stat_bar_html(total))
        cols[5].markdown(f"<span style='color:#f1f5f9;font-weight:700'>{total}</span>", unsafe_allow_html=True)

    st.caption("🔴 low → 🟠 medium → 🟢 high · colour changes continuously with the final stat")
    return values
